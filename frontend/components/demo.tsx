...
export function Demo() {
  ...
  const handleRunDemo = async () => {
    ...
    try {
      ...
      // If a file is uploaded, we should probably upload it first to get the text
      if (uploadedFile) {
      ...
      if (!content && !uploadedFile) {
        content = selectedScenario.input
      }
      ...
      // Update the current scenario view with the result
      setSelectedScenario(prev => ({
        ...prev,
        detectedPersonas: data.detected_personas.map((p: string) => p.toLowerCase()),
        output: data.risk_analysis.scored_risks.map((r: any) =>
          `${r.severity === 'CRITICAL' || r.severity === 'HIGH' ? '⚠' : '✓'} ${r.risk_summary}`
        ),
    ...
    <section className="px-4 py-24 sm:py-32 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="mb-12 text-center animate-fade-in-up">
          <h1 className="text-3xl font-bold sm:text-4xl">Guardian Interactive Demos</h1>
          <p className="mt-4 text-lg text-muted-foreground">
            See how Guardian's unified AI agent with 8 specialized personas handles complex scenarios
          </p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="space-y-3 lg:col-span-1">
            ...
            {scenarios.map((scenario, idx) => (
              <Card
                key={scenario.id}
                className={`cursor-pointer transition-all p-4 animate-slide-in ${selectedScenario.id === scenario.id
